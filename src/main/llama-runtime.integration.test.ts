...
describe("LlamaRuntime local HTTP integration", () => {
  ...
  it("collects streamed recommendation JSON before validating it", async () => {
    let recommendationPayload: Record<string, unknown> | undefined;
    const server = createServer(async (request, response) => {
      if (request.url === "/health") {
        response.writeHead(200, { "content-type": "application/json" });
        response.end('{"status":"ok"}');
      ...
      if (request.url === "/v1/chat/completions") {
        recommendationPayload = JSON.parse(await readBody(request)) as Record<string, unknown>;
        response.writeHead(200, { "content-type": "text/event-stream" });
        response.write('data: {"choices":[{"delta":{"content":"[\\\"继续聊项目\\\", "}}]}\n\n');
        response.write('data: {"choices":[{"delta":{"content":"\\\"规划今日任务\\\", \\"回顾最近问题\\\"]"}}]}\n\n');
        response.end("data: [DONE]\n\n");
        return;
      }
      response.writeHead(404);
    ...
    await expect(runtime.generateChatRecommendations("最近的历史")).resolves.toEqual([
      "继续聊项目",
      "规划今日任务",
      "回顾最近问题",
    ]);
