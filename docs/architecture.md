...
读取和更新历史。主进程仅在宠物模式、窗口未聚焦、系统持续空闲且没有聊天或语音任务时，
从最近 5 个非空会话提取有限文本，请求由应用启动的本地模型预生成 3 条开场建议并按历史
指纹缓存。Chat Panel 只读取缓存，绝不因空会话现场启动推荐推理；缓存未命中、模型未就绪
或生成失败时，界面立即使用固定建议。若用户在静默生成期间回到聊天，主进程会终止应用
自管的 llama-server 并从本地缓存重新启动，确保孤儿推理不能继续占用唯一聊天槽；模型重启
期间输入框仍可编辑，只暂时禁止发送。
